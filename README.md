...
*Fill in names and IDs before submission.*
...
> Setting this up on a new machine? **[SETUP.md](SETUP.md)** is the full walkthrough —
> installing MySQL on each OS, creating the database, wiring up credentials, moving your
> data across, and a troubleshooting section. The steps below are the short version for a
> machine that already has the four tools above.
...
│           ├── service/                # 6 services + AuthService - business rules
│           ├── controller/             # 6 REST controllers + auth + health check
│           ├── dto/                    # request DTOs for modules with foreign keys
│           ├── exception/              # GlobalExceptionHandler + 2 exception types
│           └── config/
│               ├── CorsConfig.java     # allows the Vite dev server on :5173
│               ├── FileStorageConfig.java  # serves backend/uploads/ at /uploads/
...
└── frontend/
    ...
    └── src/
        ...
        ├── components/                 # Navbar, PublicHeader, DataTable,
        │                               #   FormField, ConfirmDialog
        └── pages/
            ├── Login.jsx               # staff portal login
            ├── Dashboard.jsx
            ├── public/                 # customer site: Browse, Signup,
            │                           #   CustomerLogin, RentForm, MyBookings
            ├── customers/  categories/  vehicles/
            └── drivers/    bookings/    payments/
```

Three folders are created at runtime and are deliberately not in Git — `backend/target/`
(Maven build output), `frontend/node_modules/` (npm packages) and `backend/uploads/`
(vehicle photos). The first two are regenerated by `mvn` and `npm install`; see
[SETUP.md §9](SETUP.md#9-moving-your-existing-data-to-the-new-machine) for moving uploads
between machines.
